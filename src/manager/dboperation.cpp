...
int CreateNASList (std::map<std::string,std::string> *p_pmapNASList)
{
	...
	do {
		...
		try {
			...
			/* во врем€ исполнени€ запроса произошла ошибка */
...
int CreateSubscriberList (std::vector<SSubscriberRefresh> *p_pvectRefresh)
{
	...
	do {
		...
		try {
			...
			/* возникла ошибка выполнени€ запроса */
			LOG_E(g_coLog, "code: '%d'; message: '%s'; query '%s';", coOTLExc.code, coOTLExc.msg, coOTLExc.stm_text);
			iRetVal = coOTLExc.code;
		}
	} while (0);
	...
	do {
		if (g_mmapSessionListFull.size() > 0) {
			...
			try {
				...
				/* во врем€ выполнени€ запроса произошла ошибка */
	...
	otl_connect &p_coDBConn)
{
	...
	do {
		...
		try {
			...
			/* во врем€ выполенни€ запроса произошла ошибка */
			LOG_E(g_coLog, "code: '%d'; message: '%s'; query: '%s';", coOTLExc.code, coOTLExc.msg, coOTLExc.stm_text);
			iRetVal = coOTLExc.code;
		}
	} while (0);
	...
	bool p_bOpt)
{
	...
	do {
		...
		iRetVal = g_coConf.GetParamValue (pszConfParam, strRequest);
		if (iRetVal) {
			/* опциональный запрос не об€зательно должен быть описан в конфиге */
			if (p_bOpt) {
				iRetVal = 0;
				break;
			}
